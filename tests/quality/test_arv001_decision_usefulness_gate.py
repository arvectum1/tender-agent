...
def test_gate_rejects_generic_presence_only_report() -> None:
    ...
    assert "technical_document_present_but_no_specific_standard_or_characteristic" in result["blockers"]
    ...
    assert "security_document_present_but_security_terms_not_extracted" in result["blockers"]
...
def test_gate_accepts_concrete_material_terms_and_explicit_cap_absence() -> None:
    analysis = {
        ...
        "contract": {
            "payment": [
                {
                    "text": "Оплата осуществляется в течение 7 рабочих дней с даты подписания документа о приемке."
                }
            ],
            "security": [
                {"text": "Размер обеспечения исполнения Контракта составляет 5 % от цены Контракта."}
            ],
            "acceptance": [
                {"text": "Приемка осуществляется в течение 5 рабочих дней."}
            ],
            "liability": [
                {"text": "Пеня начисляется в размере 1/300 ключевой ставки от не уплаченной в срок суммы."}
            ],
            "liability_cap": [],
            "liability_cap_status": "not_found_in_processed_contract_text",
        },
        "application_requirements": [
            {"text": "Заявка должна содержать декларацию о соответствии участника единым требованиям."}
        ],
    }
    result = evaluate_decision_usefulness(analysis, _summary())
...
def test_gate_requires_cap_clause_when_cap_status_is_found() -> None:
    analysis = {
        ...
        "contract": {
            "payment": [{"text": "Оплата в течение 7 рабочих дней."}],
            "security": [{"text": "Обеспечение исполнения — 5 %."}],
            "acceptance": [{"text": "Приемка в течение 5 рабочих дней."}],
            "liability": [{"text": "Штраф составляет 1 процент цены Контракта."}],
            "liability_cap": [],
