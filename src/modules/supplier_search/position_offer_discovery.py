...
_PRICE_RE = re.compile(
    ...
    r"\s*(?P<currency>₽|руб(?:\.|лей|ля)?|RUB)\b",
    re.IGNORECASE,
)
...
def _supplier_label(result: YandexSearchResult) -> str:
    ...
    domain = (result.domain or urlparse(result.url).netloc).lower().replace("www.", "")
...
def discover_public_offers_for_position(
    ...
    *,
    ...
    candidates = [search_result_to_candidate(position, item) for item in response.items]
